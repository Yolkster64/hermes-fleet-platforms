...
hermes_type = st.selectbox("Hermes Type", options=["Hermes", "Ultimate X"], index=1)
hermes_variation = st.selectbox("Hermes Variation", options=["Normal", "Hybrid", "Mesh", "Deep Thinker"], index=1)
x_choice = st.selectbox(
    "X Family",
    options=["Ultimate X (auto choose)", "Ultimate X5", "Ultimate X6"],
    index=0,
)
...
default_subagents_by_profile = {
    "balanced": ["planner", "coder", "deployer"],
    "deep-analysis": ["planner", "researcher", "sql-engineer"],
    "rapid-deploy": ["planner", "coder", "deployer"],
    "safe-production": ["planner", "guardian", "deployer"],
}
subagents = st.multiselect(
    "Subagents",
    options=["planner", "researcher", "coder", "sql-engineer", "deployer", "guardian", "observer"],
    default=default_subagents_by_profile[profile],
)
    ...
    if not st.session_state["login_ok"]:
        ...
        for target in targets:
            ok, _err = _deploy_agent(
                ...
                specialty=f"{hermes_type.lower().replace(' ', '-')}-{hermes_variation.lower().replace(' ', '-')}-{x_tier}-{profile}-subs-{'-'.join(subagents[:4]) or 'default'}-agent-{target}",
                steps=steps,
                candidates=candidates,
                sql_signal=max(0.70, sql_level / 100.0),
                internet_signal=0.82,
                llm_signal=llm_signal,
                stability_bias=p["stability"],
            )
            if ok:
                ok_count += 1
...
with st.expander("Deep Guide: what to pick", expanded=False):
    st.markdown(
        ...
        "- Need fastest shipping: `Hermes + Hybrid + rapid-deploy + X5`.\n"
        "- Need deep reasoning/SQL: `Ultimate X + Deep Thinker + deep-analysis + X6`.\n"
        "- Need safest production: `Ultimate X + Mesh + safe-production + X6`.\n"
        "- Need daily default: `Hermes + Normal + balanced + auto X`.\n\n"
        "**Subagent presets:**\n"
        ...
        "**Simple steps:**\n"
        "1. Choose Hermes Type.\n"
        "2. Choose Hermes Variation.\n"
        "3. Choose profile and subagents.\n"
